...
Each deployment has an **environment file on the server** (the path is set in the deployment, relative to its server directory, `.env` by default). DevLaunch does not keep a copy: the deployment page reads the file from the server and writes it back when you edit it, so anyone deploying the project, from any Mac, sees and edits the same file. The container reads it when it starts (through `env_file` in your compose file), and for image deployments its values are handed to `docker build` too, the way a build on the server used to see a `.env` lying in the folder. Docker only honours a build argument the Dockerfile declares, so DevLaunch declares them itself in a temporary copy of the Dockerfile (an `ARG` per variable, placed just before the build step so earlier layers stay cached, and only in build stages so nothing lands in the shipped image's history). `NEXT_PUBLIC_*`, `VITE_*`, `REACT_APP_*` and friends therefore get their values with no Dockerfile changes; the deploy log warns when the Dockerfile declares such an `ARG` that the file does not set. Extras per deployment: an optional **health check URL** (polled from this Mac after the commands finish, for up to the time you set; the run only succeeds once it answers) with optional **automatic rollback** for image deployments (re-tags the previous image, reruns your commands, checks again, and reports the run as failed either way); an optional **environment file** (stored encrypted on this Mac, written to the server before your commands) and a **clean git tree check** that refuses to deploy uncommitted or un-pulled code unless you choose "Deploy anyway".
