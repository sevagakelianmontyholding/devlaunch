...
- **Server logs** — on the Servers page, every container has a Logs button that follows its output live (`docker logs -f`, last 200, 1000 or 5000 lines first); the project page has the same for the servers that project deploys to. Stop it when done; it ends by itself after an hour.
- **History** — the last 200 runs with status, kind, duration, and who ran them; click one to read its log.

The **environment file** of a deployment (stored encrypted on this Mac, written to the server before your commands) can also be **used when building the image**, which is on by default for new image deployments. That reproduces what a build on the server used to see with the file lying in the folder: every variable is handed to `docker build`, and because Docker only honours a build argument the Dockerfile declares, DevLaunch declares them itself in a temporary copy of the Dockerfile (an `ARG` per variable after each `FROM`), so `NEXT_PUBLIC_*`, `VITE_*`, `REACT_APP_*` and friends get their values with no Dockerfile changes. If the file contents in DevLaunch are empty, the file already at that path on the server is used instead. Secrets stay in the build stage; with a multi-stage Dockerfile they never reach the shipped image. Frameworks that read configuration at runtime (Nuxt's `runtimeConfig` via `NUXT_*`, for example) do not need it at all. Image deployments can additionally carry **extra build-time variables** (`NAME=value` lines passed as build arguments, overriding the file) for values only the build needs; the deploy log warns when the Dockerfile declares such an `ARG` without a value, and when an extra variable has no matching `ARG` line (Docker silently drops those). Extras per deployment: an optional **health check URL** (polled from this Mac after the commands finish, for up to the time you set; the run only succeeds once it answers) with optional **automatic rollback** for image deployments (re-tags the previous image, reruns your commands, checks again, and reports the run as failed either way); an optional **environment file** (stored encrypted on this Mac, written to the server before your commands) and a **clean git tree check** that refuses to deploy uncommitted or un-pulled code unless you choose "Deploy anyway".
...
- **Removing a project** keeps it under Recently removed, at the bottom of the Projects page, for 30 days, with its deployments, actions and history, until you restore it or delete it for good.
- **Project icons**: cards show the project's own favicon or app icon when one is found in the project folder (or its `frontend/` and similar subfolders), otherwise the live site's favicon; it is cached on this Mac. The project page has Refresh icon, Choose file and Use initials.
